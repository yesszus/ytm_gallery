...
// Creating feed
$cat_feed       = "http://www.youtube.com/api2_rest?method=youtube.videos.list_by_category_and_tag&dev_id=dyxxpOhQDf8&category_id=$import_cat&tag=$import_cat_spec&page=1&per_page=25";

// Test if feed contains information
$xml = file_get_contents("$cat_feed");
$parser = new XMLParser($xml);
$parser->Parse();
$cat_check =      ($parser->document->video_list[0]->video[0]->id[0]->tagData);
if ($cat_check == "") {
$check = "0";
}else{
$check = "1";
            	...
            	$movie_title = str_replace ("'","&#39;",$movie_title);
                  ...
                  $code_check          = $row03['movie_code'];
					...
					if (!$movie_code == $code_check){
            				...
            				$ca_i++; $text .= mysql_error();}



		}
          if ($ca_i < 1){$noimportmsg = LAN_YTM_IMPO_PREFS_13;}
	    $msgtext = "<b>$ca_i</b> " . LAN_YTM_IMPO_PREFS_2 . "$noimportmsg";
