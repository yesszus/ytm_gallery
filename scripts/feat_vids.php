...
// Creating feed
$feat_feed       = "http://www.youtube.com/api2_rest?method=youtube.videos.list_featured&dev_id=dyxxpOhQDf8";

// Test if feed contains information
$xml = file_get_contents("$feat_feed");
$parser = new XMLParser($xml);
$parser->Parse();
$feat_check =      ($parser->document->video_list[0]->video[0]->id[0]->tagData);
if ($feat_check == "") {
$check = "0";
}else{
$check = "1";
            	...
            	$movie_discr = ($video_list->description[0]->tagData);
                  ...
                  $code_check          = $row03['movie_code'];
					...
					if (!$movie_code == $code_check){
            				...
            				$ft_i++; $text .= mysql_error();}



		}
          if ($ft_i < 1){$noimportmsg = LAN_YTM_IMPO_PREFS_13;}
	    $msgtext = "<b>$ft_i</b> " . LAN_YTM_IMPO_PREFS_2 . "$noimportmsg";
